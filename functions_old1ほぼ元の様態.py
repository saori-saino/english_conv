...
# play_wav関数をStreamlit音声再生に変更（速度変更対応版）
def play_wav(audio_output_file_path, speed=1.0):
    """
    音声ファイルの読み上げ（Streamlit使用、PyAudio不要）
    Args:
        audio_output_file_path: 音声ファイルのパス
        speed: 再生速度（1.0が通常速度、0.5で半分の速さ、2.0で倍速など）
    """
    
    # 速度変更が必要な場合は、pydubで音声を変更
    if speed != 1.0:
        try:
            # 音声ファイルの読み込み
            ...
            # 変更されたオーディオを元のファイルに上書き
            modified_audio.export(audio_output_file_path, format="wav")
        except Exception as e:
            st.warning(f"音声速度の変更に失敗しました（通常速度で再生します）: {e}")
